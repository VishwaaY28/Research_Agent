...
const ContentSources: React.FC = () => {
  ...
  const normalizeUrl = (url: string) => {
    if (!url) return url;
    // If URL looks like a server-local path (e.g. 'source_files/filename.pdf' or '\\source_files\\...'),
    // convert it to an absolute URL pointing to the server static mount we created (/source_files)
    const normalizedPath = url.replace(/\\/g, '/').replace(/^\/+/, '');
    if (normalizedPath.startsWith('source_files/') || normalizedPath.startsWith('/source_files/')) {
      const parts = normalizedPath.split('/');
      const filename = parts[parts.length - 1];
      const apiBase = API.BASE_URL ? API.BASE_URL() : '';
      const origin =
        apiBase && apiBase.length > 0 ? apiBase.replace(/\/$/, '') : window.location.origin;
      return `${origin}/source_files/${encodeURIComponent(filename)}`;
    }
    try {
      const parsed = new URL(url);
      return parsed.toString();
    } catch (e) {
      // If URL constructor fails, try prefixing https://
      return `https://${url}`;
    }
  };
  ...
  const openUrlInNewTab = (url: string) => {
    ...
    // Try using anchor navigation first to preserve expected behavior
    try {
    ...
    <div className="min-h-screen bg-white px-4 md:px-8">
      ...
      <div className="px-2 py-4">
        {filteredSources.length === 0 ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 overflow-visible shadow-sm mt-2">
              ...
              {filteredSources.map((source) => (
                ...
                >
                  ...
                  >
                    {source.source_url ? (
                      <button
                        onClick={() => openUrlInNewTab(source.source_url)}
                  ...
                  <div className="col-span-2 flex justify-center relative three-dot-menu overflow-visible">
                    <button
                    ...
                    {menuOpenId === source.id && (
                      <div
                        className="absolute right-0 top-8 z-50 w-32 bg-white border border-gray-200 rounded shadow-md py-1"
                        style={{ minWidth: 140 }}
                      >
                        <button
                          onClick={() => {
