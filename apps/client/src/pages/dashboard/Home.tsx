    ...
    const fetchRecentContent = async () => {
      if (dashboardData?.recent_workspaces && dashboardData.recent_workspaces.length > 0) {
        try {
          ...
          // Attach workspace id to each item so we can navigate to the correct workspace
          const annotated = results
            .flatMap((items, idx) =>
              items.map((item) => ({ ...(item as any), workspace_id: workspacesToFetch[idx].id })),
            )
            .sort(
              (a: any, b: any) =>
          ...
          setRecentContent(annotated as GeneratedContent[]);
        } catch (err) {
                ...
                <div className="space-y-2">
                  {recentContent.map((content) => (
                    <div
                      key={content.id}
                      onClick={() => {
                        if ((content as any).workspace_id) {
                          navigate(
                            `/dashboard/workspaces/${(content as any).workspace_id}?openGenerated=${content.id}`,
                          );
                        }
                      }}
